    ...
    <div className="w-full h-full bg-white rounded-lg flex flex-col gap-3 p-4 max-h-[70vh] overflow-y-auto">
      {items.length === 0 ? (
        ...
        <>
          ...
          <div key={index} className="w-full h-30 p-2 shadow-md flex justify-between gap-5 rounded-lg transition-all duration-300" style={{backgroundColor: 'var(--bg-warm)', border: '1px solid var(--border-light)'}}>
            <div className="w-[60%] h-full bg-white flex gap-5 rounded-lg overflow-hidden">
              <div className="w-[40%] h-full overflow-hidden">
                <img src={item.image} alt={item.name} className="object-cover h-full w-full" />
              </div>
              <div className="w-[60%] flex flex-col gap-2 py-2">
                ...
                <div className="w-27.5 h-10 flex rounded-lg overflow-hidden shadow-md font-semibold border-2" style={{borderColor: 'var(--coral-primary)'}}>
                  <button
                    onClick={() => handleDecrease(item)}
                    className="w-[30%] h-full flex justify-center items-center font-bold text-xl bg-white transition-all duration-300 ease-in-out hover:scale-110"
                  ...
                  <span className="w-[40%] h-full flex justify-center items-center" style={{backgroundColor: 'var(--border-light)', color: 'var(--coral-primary)'}}>
                    {item.quantity}
                  </span>
                  <button
            ...
            <div className="flex flex-col justify-start items-end gap-6">
              ...
              <RiDeleteBin6Line 
                ...
                className="w-7.5 h-6 text-xl cursor-pointer transition-all duration-300 hover:scale-110" 
                style={{color: '#d32f2f'}}
              />
            </div>
          </div>
