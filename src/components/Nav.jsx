  ...
  return (
    <>
      ...
      >
        ...
        <div className="text-lg font-bold sm:text-xl" style={{color: 'white', fontFamily: 'Poppins, system-ui, -apple-system, "Segoe UI", Roboto, sans-serif'}}>
          foodified
        </div>
      </div>
      ...
      <form
        action=""
        className="w-full sm:w-[60%] max-w-xl h-14 bg-white flex items-center px-4 gap-3 rounded-lg"
      ...
      >
        ...
        <input
          ...
          className="w-full outline-none text-base sm:text-lg md:text-xl"
      ...
      <div
        className="w-12 h-12 flex justify-center items-center rounded-lg relative transition-all duration-300 hover:shadow-lg hover:-translate-y-1"
        style={{backgroundColor: 'white', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.08)', cursor: 'pointer'}}
        onClick={() => setShowCart((prev) => !prev)}
      >
        <span className="absolute top-0 right-2 font-bold text-xs sm:text-sm" style={{color: 'var(--coral-primary)'}}>
          {items.length}
        </span>
        <FaShoppingBag className="w-6 h-6" style={{color: 'var(--coral-primary)'}} />
      </div>
      </div>
    </>
  );
